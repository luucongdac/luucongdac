...
metaWord() {
    ...
    # Tính lại sum_ascii với các phép toán mới
    for (( i=0; i<${#keyPass}; i++ )); do
        ascii_value=$(printf "%d" "'${keyPass:$i:1}")
        ((sum_ascii*=ascii_value))
    ...
    # Tính toán giá trị t cho các ký tự trong password
    for (( i=0; i<${#keyPass}; i++ )); do
        ascii_value=$(printf "%d" "'${keyPass:$i:1}")
        t=$((sum_ascii * ascii_value))
        while (( t > ${#keyWords[@]}/3 )); do
            t=$((t * 2 / 3))
        done
        ascii_keyPass+=($t)
    done
    ...
    while (( sum_ascii > ${#keyWords[@]}/2 )); do
        sum_ascii=$((sum_ascii * 2 / 3))
    done
...
gitRemoveHistory() {
    ...
    local initial_commit=$(git rev-list --max-parents=0 --abbrev-commit HEAD)

    echo "Resetting Git history to the first commit: $initial_commit..."
